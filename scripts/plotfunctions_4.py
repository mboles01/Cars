...
def plot_combo_depr2(data, depr_summary, model, model_counts, save): 
    ...
    xmax = int(max(segment['Half life'])) + 2
    xlabel = 'Half life (years)'
    ylabel = 'Counts'
    
    bins = np.arange(round(min(data_halflife),1), max(data_halflife) + binwidth, binwidth)
    ...
    textbox3 = r'The $%s$ $%s$ loses half its value every' % (make, model)
    
    textbox3_right = '%.2f years' % user_choice_halflife
    ...
    if user_choice_halflife > segment_average:
        ...
        if user_choice_halflife == max(segment['Half life']):
             ...
             textbox5_right = '%.2f years\n %.2f years\n %.2f years\n ' % (
                segment.sort_values('Half life', ascending=False).iloc[0][['Half life']][0],
    ...
    ax1.text(1, 1.80, textbox3_right, 
            transform = ax2.transAxes, 
            fontsize = 24,
            weight = 'bold', 
            horizontalalignment = 'right',
    ...
    ax1.text(1, 1.65, textbox4_right, 
            transform = ax2.transAxes, 
            fontsize = 24,
            weight = 'bold', 
            horizontalalignment = 'right',
    ...
    ax1.text(1, 1.425, textbox5_right, 
            transform = ax2.transAxes, 
            fontsize = 24,
            fontname = 'Helvetica', 
            horizontalalignment = 'right',
    ...
    # save figure
    if save == True:
        figure_name = '../images/depr_by_model_3/' + str(model) + '.png'
