...
def plot_depr_age(data, model, newerthan, b_lower, counter, counts, alpha, save): # fit_data
    ...
    popt_2, pcov_2 = curve_fit(exp_function, year_age_median_price['Age'], 
                        year_age_median_price['Median Price'], 
                        absolute_sigma=False, maxfev=1000,
                        bounds=((35400, b_lower), (35500, 1)))

    print(popt[0], popt_2[0])
    print(0.6931/popt[1], 0.6931/popt_2[1])
    ...
    # plot fit
    x_axis_smooth = np.arange(0, max(year_age_median_price['Age'])+1, .1)
    plt.plot(x_axis_smooth, exp_function(x_axis_smooth, *popt), '#ff4c00', linestyle=':', linewidth=3)
    plt.plot(x_axis_smooth, exp_function(x_axis_smooth, *popt_2), '#ff4c00', linestyle='--', linewidth=3)    
    ...
    textbox_4 = '$a$ = %5.0f \n$b$ = %0.3f' % (popt_2[0], popt_2[1]) + '\n$R^{2}$ = %5.2f' % 0.51
    textbox_5 = '$Half$-$life:$ ' + str(round(0.6931/popt_2[1], 2)) + ' $y$'
    
    
    ax.text(0.54, 0.95, textbox_1, transform = ax.transAxes, fontsize = 18, 
            fontname = 'Helvetica', verticalalignment = 'top', bbox = props_1)
    
    ax.text(0.25, 0.95, textbox_2, transform = ax.transAxes, fontsize = 18, 
            fontname = 'Helvetica', verticalalignment = 'top', bbox = props_2)
    
    ax.text(0.25, 0.75, textbox_3, transform = ax.transAxes, fontsize = 18, 
            fontname = 'Helvetica', verticalalignment = 'top', bbox = props_2)
   
    ax.text(0.05, 0.45, textbox_4, transform = ax.transAxes, fontsize = 18, 
            fontname = 'Helvetica', verticalalignment = 'top', bbox = props_2)
    
    ax.text(0.05, 0.25, textbox_5, transform = ax.transAxes, fontsize = 18, 
            fontname = 'Helvetica', verticalalignment = 'top', bbox = props_2)
